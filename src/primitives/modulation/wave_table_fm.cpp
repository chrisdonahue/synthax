...
void synthax::primitive::modulation::wave_table_fm::evaluateBlockPerformance(unsigned firstFrameNumber, unsigned numSamples, float* sampleTimes, unsigned numConstantVariables, float* constantVariables, float* buffer) {
...
void synthax::primitive::modulation::wave_table_fm::set_render_info(float sample_rate, unsigned block_size, unsigned max_frame_number, float max_frame_start_time) {
	node::set_render_info(sample_rate, block_size, max_frame_number, max_frame_start_time);
	freq_nyquist = sample_rate / 2;
}

void synthax::primitive::modulation::wave_table_fm::update_mutated_params() {
