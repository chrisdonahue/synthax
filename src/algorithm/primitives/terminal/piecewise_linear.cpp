...
synthax::node::terminal::piecewise_linear::piecewise_linear(param* splinetype, GPMutatableParam* numsegments, std::vector<GPMutatableParam*>* pointsOrParams)
{
...
synthax::node::terminal::piecewise_linear::~piecewise_linear() {
    done_rendering();
}
...
synthax::node::terminal::piecewise_linear* synthax::node::terminal::piecewise_linear::get_copy() {
    ...
    return new piecewise_linear(params[0]->get_copy(), mutatableParams[1]->getCopy(), paramCopies);
}

void synthax::node::terminal::piecewise_linear::evaluateBlockPerformance(unsigned firstFrameNumber, unsigned numSamples, float* sampleTimes, unsigned numConstantVariables, float* constantVariables, float* buffer) {
